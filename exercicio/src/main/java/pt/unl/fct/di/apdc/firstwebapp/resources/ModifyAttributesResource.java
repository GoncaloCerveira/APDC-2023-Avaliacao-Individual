...
public class ModifyAttributesResource {
			...
			if (isModifyAttributesValid(user.getString("role"), username.getString("role"), data.user, data.username)) {
				switch (user.getString("role")) {
					case "USER":
						...
						break;
					case "GBO":
						if(!username.getString("role").equals("USER")) {
							txn.rollback();
							LOG.warning("ModifyAttributes operation canceled, " 
									+ data.user + " does not have the permissions to modify.");
						...
						break;
					case "GA":
						if ((data.role.equals("GBO") && !username.getString("role").equals("USER"))
								|| (data.state != null && !username.getString("role").equals("GBO"))) {
							txn.rollback();
							LOG.warning("ModifyAttributes operation canceled, " 
									+ data.user + " does not have the permissions to modify.");
						...
						break;
					case "GS":
						if ((data.role.equals("GBO") && !username.getString("role").equals("GA"))
								|| (data.state != null && !username.getString("role").equals("GBO"))) {
							txn.rollback();
							LOG.warning("ModifyAttributes operation canceled, " 
									+ data.user + " does not have the permissions to modify.");
	...
	private boolean isModifyAttributesValid(String userRole, String usernameRole, String user, String username) {
		...
		if(userRole.equals("GBO") && (user.equals(username) || usernameRole.equals("USER")))
			return true;
		
		if(userRole.equals("GA") && (user.equals(username) || userRole.equals("GA") 
				&& (username.equals("GBO") || username.equals("USER"))))
			return true;
		
		if(userRole.equals("GS") && user.equals(username) || userRole.equals("GS") 
				&& (username.equals("GBO") || username.equals("GA")))
			return true;
		return false;
	}
